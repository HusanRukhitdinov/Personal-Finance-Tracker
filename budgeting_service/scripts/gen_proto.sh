...
for dir in "${CURRENT_DIR}/personal_finance_tracker"/*; do
    ...
    for proto_file in "$dir"/*.proto; do
        ...
        protoc -I="${dir}" -I="${CURRENT_DIR}/personal_finance_tracker" -I /usr/local/include --go_out="$OUTPUT_DIR" --go-grpc_out="$OUTPUT_DIR" "$proto_file" || { echo "Failed to compile $proto_file"; exit 1; }
    done
done
