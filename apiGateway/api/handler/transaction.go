...
func (h *Handler) CreateTransactionHandler(ctx *gin.Context) {
	...
	// Bind JSON request body to the request object
	if err := ctx.ShouldBindJSON(&request); err != nil {
		handleResponse(ctx, h.log, "error while reading body", http.StatusBadRequest, err.Error())
		return
	}

	// Format the date with default time and timezone offset
	...
	// Prepare the request object for RabbitMQ message
	transactionMessage := pb.TransactionRequest{
...
func (h Handler) GetAllTransactionsHandler(c *gin.Context) {
	...
	response, err := h.services.TransactionService().GetListTransactions(context.Background(), &pb.GetListRequest{
		Page:   int32(page),
		Limit:  int32(limit),
		Search: search,
	})
...
func (h *Handler) GetUserIncomeHandler(ctx *gin.Context) {
	...
	request := pb.GetUserMoneyRequest{
		UserId:    userID,
